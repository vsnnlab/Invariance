function [resp_z_mat,face_resp_z_mat,max_resp_z_mat] = fun_InvRange_Resp(act_target,num_target_cell,cls_idx,var_idx)

resp_z_mat = zeros(num_target_cell,13,9,200);
face_resp_z_mat = zeros(num_target_cell,13,200);
max_resp_z_mat = zeros(num_target_cell,13,200);

for nn = 1:num_target_cell
    for vv = 1:13
        resp_temp = zeros(9,200);
        for cc = 1:9
            idx_tmp = (cls_idx == cc) & (var_idx == vv);
            resp_temp(cc,:) = act_target(nn,idx_tmp);
        end
